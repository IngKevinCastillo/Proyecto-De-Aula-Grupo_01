...
public class Menu {
   ...
   public void EliminarMesa(){
    ...
    public void Informe(){
        /*int numSala;
        dueno.inicializarDatos(new Datos());
        
        System.out.print("Digite la sala a generar informe:"); numSala = entrada.nextInt();
        if(dueno.buscarsala(numSala) != null){
            for(Recepcionista rec : dueno.getEmpleado()){
                Recepcionista recep = rec;
                Sala salaa = dueno.buscarsala(numSala);
                dueno.generarInforme(salaa, recep);
            }
        }else{
            System.out.println("La sala no exite");
        }*/
    }
